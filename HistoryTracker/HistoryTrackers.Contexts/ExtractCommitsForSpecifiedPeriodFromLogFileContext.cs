...
namespace HistoryTracker.Contexts
{
    public class ExtractCommitsForSpecifiedPeriodFromLogFileContext
    {
        ...
        public ExtractCommitsForSpecifiedPeriodFromLogFileContext(IExtractCommitsForSpecifiedPeriodFromLogFileGateway gateway)
        {
            _gateway = gateway;
        }
        public ExtractAllCommitsForSpecifiedPeriodResponse Execute(string logFilePath, string endDatePeriod)
        {
            ...
            if (!commitsBeforeEndDate.Contains(commitToAdd) && IsCommitInSpecificPeriod(commitToAdd.CommitDate, endDatePeriod))
                commitsBeforeEndDate.Add(commitToAdd);

            if(!commitsAfterEndDate.Contains(commitToAdd) && !IsCommitInSpecificPeriod(commitToAdd.CommitDate,endDatePeriod))
                commitsAfterEndDate.Add(commitToAdd);

            return new ExtractAllCommitsForSpecifiedPeriodResponse { IsSuccess = true, CommitsBeforeEndDate = commitsBeforeEndDate, CommitsAfterEndDate = commitsAfterEndDate};
        }
        ...
        {
            if (DateTime.Parse(commitDateTime, CultureInfo.InvariantCulture) <= DateTime.Parse(endDatePeriod, CultureInfo.InvariantCulture))
                return true;
            return false;
        }
    }
